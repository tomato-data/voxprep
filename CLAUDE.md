...
이 파일은 Claude Code가 이 리포지토리에서 작업할 때 따르는 지침입니다. **학습 목적 프로젝트**이자 **실제로 돌아가는 CLI를 끝까지 가져가는 프로젝트**입니다. 둘 중 어느 쪽도 구실이 아닙니다 — 사용자 손에 쥐어지는 CLI를 짓는 경험 자체가 이 프로젝트의 핵심 산출물이며, 그 경험을 통해 Typer/TDD/ODP를 체화하는 것이 병렬 목표입니다.
...
| 패키지/프로젝트 관리 | [uv](https://github.com/astral-sh/uv) | `uv sync` / `uv run` / `uv add`. `pyproject.toml`은 `[dependency-groups].dev`로 dev deps 선언 |
| 가상환경 | `voxprep/.venv` (리포 루트, uv 관리) | Phase 01에서 신규 생성. Python은 `.python-version`으로 3.12 고정. GPT-SoVITS 스택(faster-whisper 등)은 Phase 04 도달 시 `uv add`로 이 venv에 추가 |
| 락파일 | `uv.lock` (커밋) | 재현성 보장 — 재생성은 `rm -rf .venv uv.lock && uv sync` |

---

## 명령어

`uv run <cmd>`는 venv를 자동 활성화한 상태로 명령을 실행합니다. `source .venv/bin/activate`가 필요 없어요. 그래도 쉘에서 직접 `pytest`/`voxprep`을 치고 싶으면 수동 activate 해도 OK.

```bash
# 의존성 동기화 (최초 세팅, 의존성 변경 후)
uv sync

# 의존성 추가 (pyproject.toml + uv.lock 자동 갱신)
uv add faster-whisper           # Phase 04에서
uv add --dev ruff               # dev 그룹으로 추가

# 테스트
uv run pytest tests/ -v
uv run pytest tests/unit/test_list_parser.py::test_parses_single_line -v

# CLI
uv run voxprep --help
uv run voxprep version
uv run voxprep slice <input_dir> <output_dir>
uv run voxprep asr <output_dir>
uv run voxprep review <list_file>
uv run voxprep prep <raw_dir>

# (선택) 쉘 세션 전체에 venv 걸어두고 싶을 때
