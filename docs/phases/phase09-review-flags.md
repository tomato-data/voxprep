...
> **출처**: `voxprep/GPT-SoVITS/docs/ko/SETUP_GUIDE.md` 3-4절 "ASR 결과 정리". 사용자가 ASMR 한국어 데이터셋을 직접 처리하면서 정의한 제거 대상 목록입니다. voxprep `--auto-prune`은 이 규칙을 자동화한 것입니다.
...
- **`non_korean_noise`의 임계값**: 한글 비율 50% 기준은 임의값. 사용자 데이터셋(ASMR)에 한자/영문이 섞여 있을 가능성 — 처음 돌릴 때 false positive가 많으면 회고에 임계값 조정 기록.
