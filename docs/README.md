...
| 19 | [phase19-odp-refinement.md](phases/phase19-odp-refinement.md) | ODP 46 규칙 기반 리팩터 (8 sub-phases) | (구조 개선) | ⏳ |
| 20 | [phase20-config-file.md](phases/phase20-config-file.md) | 기본값 config 파일 + CLI 오버라이드 체계 | `voxprep config {show,init,edit,path}` | ⏳ |
...
| 14 | [phase14-train.md](phases/phase14-train.md) | `train` — SoVITS + GPT 학습 | `voxprep train {sovits,gpt,all}` | ✅ |
...
엔드-투-엔드 사용법, 설치, 명령어 레퍼런스는 [**`docs/GUIDE.md`**](GUIDE.md) 참조. Phase 문서들은 구현 과정(TDD/이식 결정)을 담고, GUIDE 는 완성된 CLI 를 어떻게 쓰는지 다룹니다.

## 아키텍처 개요

전체 객체 구성(VO/Entity/Service), 레이어, 모듈 간 연결 다이어그램은 [**`docs/ARCHITECTURE.md`**](ARCHITECTURE.md) 참조. ODP 분류 관점에서 voxprep이 왜 지금처럼 나뉘었는지를 설명합니다.
